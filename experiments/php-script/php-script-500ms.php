...
class benchmark {
	...
	private static function test_Math($count = 280000) {
	...
	private static function test_StringManipulation($count = 260000) {
		$time_start = microtime(true);
	...
	private static function test_Loops($count = 38000000) {
		$time_start = microtime(true);
		for($i = 0; $i < $count; ++$i);
		$i = 0; while($i < $count) ++$i;
		return number_format(microtime(true) - $time_start, 3);
	...
	private static function test_IfElse($count = 18000000) {
		$time_start = microtime(true);
		for ($i=0; $i < $count; $i++) {
			if ($i == -1) {
			} elseif ($i == -2) {
