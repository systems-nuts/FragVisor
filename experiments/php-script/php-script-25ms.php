...
class benchmark {
	...
	private static function test_Math($count = 1400) {
	...
	private static function test_StringManipulation($count = 1300) {
		$time_start = microtime(true);
	...
	private static function test_Loops($count = 190000) {
		$time_start = microtime(true);
		for($i = 0; $i < $count; ++$i);
		$i = 0; while($i < $count) ++$i;
		return number_format(microtime(true) - $time_start, 3);
	...
	private static function test_IfElse($count = 90000) {
		$time_start = microtime(true);
		for ($i=0; $i < $count; $i++) {
			if ($i == -1) {
			} elseif ($i == -2) {
